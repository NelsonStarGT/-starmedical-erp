...
export default async function ReceptionCompaniesPage() {
  ...
  if (!siteId) {
    return (
      <div className="rounded-xl border border-rose-100 bg-rose-50 p-4 text-sm text-rose-700">
        Selecciona una sede activa para ver pacientes de empresas.
      </div>
    );
  }

  const snapshot = await actionGetReceptionWorklist({
    siteId,
    filters: {
      companyOnly: true
    }
