...
export default async function UsuariosConfiguracionPage() {
  const user = await requireUsersAdminPageAccess();
  const [meta, policy] = await Promise.all([
    getUsersAdminMeta(),
    getTenantSecurityPolicy(user.tenantId || "global")
  ]);

  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
        Esta vista ya no usa catálogos locales. Roles, sucursales y política de credenciales salen de tablas persistidas.
      </div>
