...
export async function GET(req: NextRequest) {
  try {
    ...
    const branchScope = enforceAgendaBranchScope(auth.user, branchId);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const branchScope = enforceAgendaBranchScope(auth.user, body?.sucursalId);
    if (!branchScope.allowed) {
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const branchScopeCurrent = enforceAgendaBranchScope(auth.user, before.branchId);
    ...
    const parsed = await toPrismaInput(body, auth.user!.id);
    const branchScopeTarget = enforceAgendaBranchScope(auth.user, parsed.branchId);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const branchScope = enforceAgendaBranchScope(auth.user, before.branchId);
    if (!branchScope.allowed) {
