...
export async function POST(req: NextRequest) {
  ...
  try {
    const config = await prisma.attendanceIntegrationConfig.findFirst();
    ...
    await prisma.attendanceIntegrationConfig.update({
      where: { id: config.id },
    ...
    const config = await prisma.attendanceIntegrationConfig.findFirst();
    if (config) {
      await prisma.attendanceIntegrationConfig
        .update({
          where: { id: config.id },
