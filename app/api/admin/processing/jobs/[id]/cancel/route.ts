...
export async function POST(
  ...
  try {
    ...
    const response = await requestTenantProcessingService({
      tenantId,
      method: "POST",
      pathname: `/jobs/${encodeURIComponent(jobId)}/cancel`,
      body: {},
    ...
    if (!response.ok) {
      ...
      if (msg.includes("cannot_cancel") || msg.includes("conflict") || msg.includes("already")) {
        return NextResponse.json({ ok: false, code: "CONFLICT", error: "El job no puede cancelarse." }, { status: 409 });
      }
      return NextResponse.json(
        {
          ok: false,
          code: "PROCESSING_SERVICE_ERROR",
          error: response.error || "No se pudo cancelar el job."
        },
        { status: 502 }
      );
    }
