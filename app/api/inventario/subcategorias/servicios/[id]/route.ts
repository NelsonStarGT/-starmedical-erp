...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const current = await prisma.serviceSubcategory.findFirst({ where: inventoryWhere(scope, { id: params.id }) });
    if (!current) return NextResponse.json({ error: "Subcategoría no encontrada" }, { status: 404 });
    const updated = await prisma.serviceSubcategory.update({ where: { id: current.id }, data });
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (sub._count.services > 0) return NextResponse.json({ error: "No se puede eliminar: tiene servicios asociados" }, { status: 400 });
    await prisma.serviceSubcategory.update({ where: { id: sub.id }, data: { deletedAt: new Date(), status: "Inactivo" } });
