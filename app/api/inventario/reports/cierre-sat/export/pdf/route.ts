...
export async function GET(req: NextRequest) {
  ...
  try {
    const buffer = await generateCierreSatPdf({ tenantId: scope.tenantId, dateFrom, dateTo, branchId });
    return new NextResponse(buffer, {
      status: 200,
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename=\"cierre-sat-${dateFromRaw}-${dateToRaw}.pdf\"`
      }
    });
  } catch (err) {
    console.error(err);
