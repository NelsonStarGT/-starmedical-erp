...
export async function POST(req: NextRequest, { params }: { params: { kind: string } }) {
  ...
  try {
    ...
    const branchId = (form.get("branchId") as string) || req.nextUrl.searchParams.get("branchId") || undefined;
    ...
    const result = await processImport(kind, buffer, { preview: previewFlag, branchId, userId });
