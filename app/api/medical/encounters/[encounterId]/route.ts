...
import { getEncounterRecord, listEncounterDocuments } from "@/lib/medical/encounterRealStore";
...
export async function GET(req: NextRequest, ctx: { params: { encounterId: string } }) {
  ...
  try {
    ...
    const documents = await listEncounterDocuments(encounterId);
    return NextResponse.json({ ok: true, data: { ...encounter, documents } });
  } catch (error) {
    const message = error instanceof Error ? error.message : "No se pudo obtener encounter";
