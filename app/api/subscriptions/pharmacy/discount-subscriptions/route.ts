...
import {
  createDiscountSubscription,
  listDiscountSubscriptions
...
export async function GET(req: NextRequest) {
  ...
  const parsed = listDiscountSubscriptionsQuerySchema.safeParse({
  ...
  try {
    const data = await listDiscountSubscriptions(parsed.data, auth.user);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const payload = createDiscountSubscriptionSchema.parse(json);
    const data = await createDiscountSubscription(payload, auth.user);
