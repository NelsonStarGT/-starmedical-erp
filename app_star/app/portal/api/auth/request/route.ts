...
export async function POST(req: NextRequest) {
  ...
  if (!dpi || !phone) {
    return NextResponse.json(
      { ok: false, error: "Ingresa DPI y teléfono." },
      { status: 400 }
    );
  }

  const requestMeta = readPortalRequestMeta(req.headers);
  const requestedDestination = phone;
  ...
  const personEmail = person ? normalizeEmail(person.email) : null;
  const deliverToEmail = personEmail && (!email || email === personEmail) ? personEmail : null;
  const channel = deliverToEmail ? "email" : "none";
  let delivery: "email" | "none" = deliverToEmail ? "email" : "none";
  const challengeDestination = phone;
  ...
  const nextStepUrl = `/portal/auth/verify?phone=${encodeURIComponent(phone)}`;
  ...
  if (person && deliverToEmail) {
    try {
      await sendMail({
        ...
        tenantId: (person as { tenantId?: string | null } | null)?.tenantId || "global",
        emailType: "portal-otp",
        moduleKey: "ADMIN"
      });
    } catch (error) {
      delivery = "none";
