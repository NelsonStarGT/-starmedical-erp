...
export async function POST(req: NextRequest) {
  ...
  if (employee.status === "TERMINATED") {
    return NextResponse.json({ error: "Empleado no elegible (estado terminado)" }, { status: 409 });
  }
  ...
  const log = await prisma.timeClockLog.create({
    data: {
      ...
      legalEntityId,
      timestamp,
      type: (parsed.data.type as any) || TimeClockLogSource.MANUAL,
  ...
  return NextResponse.json({ data: serializeLog(log as any) }, { status: 201 });
}
