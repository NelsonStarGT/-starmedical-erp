...
import { buildActiveEmployeesWhere } from "@/lib/hr/filters";
...
async function handleGet(req: NextRequest) {
  ...
  const where: Prisma.HrEmployeeWhereInput = buildActiveEmployeesWhere({ sessionUser: auth.user });
  ...
  const [total, employees] = await prisma.$transaction([
    ...
    prisma.hrEmployee.findMany({
      ...
      include: employeeIncludeFor(auth.user),
  ...
  return NextResponse.json({
    data: employees.map((employee) => serializeEmployee(employee, auth.user)),
    meta: {
      page,
      pageSize: PAGE_SIZE,
      total,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (isDraft) {
      ...
      const draft = await prisma.hrEmployee.create({
        data: {
          ...
          biometricId: parsedDraft.biometricId?.trim() || null,
    ...
    if (parsed.biometricId) {
      if (!hasPermission(auth.user, "USERS:ADMIN")) {
        return NextResponse.json({ error: "Solo administradores pueden asignar ID biométrico" }, { status: 403 });
      }
      const existingBiometric = await prisma.hrEmployee.findFirst({ where: { biometricId: parsed.biometricId.trim() } });
    ...
    const employee = await prisma.$transaction(async (tx) => {
      ...
      const saved = await tx.hrEmployee.create({
        data: {
          ...
          biometricId: cleanNullableString(parsed.biometricId),
      ...
      return tx.hrEmployee.findUnique({ where: { id: saved.id }, include: employeeIncludeFor(auth.user) });
    });

    return NextResponse.json({ data: serializeEmployee(employee!, auth.user) }, { status: 201 });
  } catch (err: any) {
