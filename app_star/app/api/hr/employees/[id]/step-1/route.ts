...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } } | { params: Promise<{ id: string }> }) {
  ...
  if (parsed.data.biometricId) {
    ...
    const existingBio = await prisma.hrEmployee.findFirst({
      where: { biometricId: parsed.data.biometricId.trim(), id: { not: resolvedParams.id } }
    });
    if (existingBio) return NextResponse.json({ error: "ID biométrico ya asignado" }, { status: 409 });
  }
  ...
  const updated = await prisma.$transaction(async (tx) => {
    const saved = await tx.hrEmployee.update({
      ...
      data: {
        ...
        biometricId: cleanNullableString(parsed.data.biometricId),
