...
async function getHandler(req: NextRequest) {
  ...
  const lastUpdatedBy = (config as any)?.retryPolicy?.lastUpdatedBy || null;
  return NextResponse.json({ data: { ...config, lastUpdatedBy } });
}
...
async function postHandler(req: NextRequest) {
  ...
  const currentRetry = (existing?.retryPolicy as any) || {};
  const lastUpdatedBy = { id: auth.user.id, name: auth.user.name, email: auth.user.email };
  const payload = {
    ...data,
    updatedAt: new Date(),
    retryPolicy: { ...currentRetry, lastUpdatedBy }
  };
  ...
  return NextResponse.json({ data: { ...saved, lastUpdatedBy } });
}
