...
async function handler(req: NextRequest) {
  ...
  const allowed = ["MEMBERSHIPS:ADMIN", "MEMBERSHIPS:READ", "MEMBERSHIPS:PLANS:READ"].some((perm) => hasPermission(auth.user, perm));
  if (!allowed) return NextResponse.json({ error: "No autorizado" }, { status: 403 });

  const plans = await prisma.membershipPlan.findMany({
    orderBy: [{ createdAt: "asc" }],
    include: { MembershipBenefit: true }
  });
  const config = await getMembershipConfig(prisma);

  return NextResponse.json({ data: { plans, config } });
}
