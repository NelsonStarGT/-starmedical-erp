...
import { inferPhoneIso2ByCountryText, normalizePhoneToE164, type PhoneCountryCodeConfig } from "@/lib/phone/normalize";
...
async function listActivePhoneCountryCodes(): Promise<PhoneCountryCodeConfig[]> {
  ...
  return rows as PhoneCountryCodeRow[];
}

async function resolvePreferredPhoneIso2(input: {
  explicitIso2?: string | null;
  geoCountryId?: string | null;
  countryText?: string | null;
  catalog: PhoneCountryCodeConfig[];
}) {
  const explicitIso2 = normalizeOptional(input.explicitIso2)?.toUpperCase() ?? null;
  if (explicitIso2 && input.catalog.some((item) => item.iso2.toUpperCase() === explicitIso2)) {
    return explicitIso2;
  }
  ...
  if (geoCountryId) {
    const geoCountry = await prisma.geoCountry.findUnique({
      where: { id: geoCountryId },
      select: { iso2: true }
    });
    const iso2 = geoCountry?.iso2?.toUpperCase() ?? null;
    if (iso2 && input.catalog.some((item) => item.iso2.toUpperCase() === iso2)) {
      return iso2;
    }
  }

  return inferPhoneIso2ByCountryText(input.countryText, input.catalog);
}

async function normalizePhoneE164ForWrite(input: {
  ...
  const rawPhone = normalizeOptional(input.rawPhone);
  if (!rawPhone) {
    if (input.required) throw new Error(`${input.fieldLabel || "Teléfono"} requerido.`);
    return null;
  }

  const catalog = await listActivePhoneCountryCodes();
  const preferredIso2 = await resolvePreferredPhoneIso2({
    explicitIso2: input.explicitIso2,
    geoCountryId: input.geoCountryId,
    countryText: input.countryText,
    catalog
  });

  const normalized = normalizePhoneToE164(rawPhone, catalog, {
    preferredIso2,
    required: Boolean(input.required),
    fieldLabel: input.fieldLabel || "Teléfono"
  });
...
export async function actionCreatePersonClient(input: {
  ...
  const phoneRaw = normalizeRequired(input.phone, "Teléfono requerido.");
  ...
  const phoneE164 = await normalizePhoneE164ForWrite({
    rawPhone: phoneRaw,
    required: true,
    explicitIso2: input.phoneCountryIso2,
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (addressGeneral) locations.push({ type: ClientLocationType.GENERAL, address: addressGeneral, isPrimary: true });
      ...
      if (locations.length) {
        await tx.clientLocation.createMany({
          data: locations.map((loc) => ({
            ...
            department: loc.type === ClientLocationType.GENERAL ? geo.admin1Name : null,
            city: loc.type === ClientLocationType.GENERAL ? geo.admin2Name : null,
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.clientLocation.create({
        data: {
          clientId: created.id,
          type: ClientLocationType.BUSINESS,
          address,
          city,
          department,
          country,
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      await logClientAuditTx(tx, {
        ...
        clientId: created.id,
        action: "CLIENT_CREATED",
        metadata: { type: "INSTITUTION", institutionTypeId }
      });
...
export async function actionCreateInsurerClient(input: {
  ...
  const companyName = normalizeRequired(input.name, "Nombre de aseguradora requerido.");
  const nit = normalizeOptional(input.nit);
  ...
  const rows = await prisma.clientProfile.findMany({
    where: {
      ...
      OR: [
        ...
        { phoneE164: { contains: q, mode: insensitiveMode } },
        ...(includePerson
          ? [
...
async function resolveGeoDivisionFromAdminLikeId(input: {
  id: string;
  level: 1 | 2 | 3;
  onlyActive?: boolean;
}) {
  ...
  const whereLevel =
    level === 1
      ? [{ id }, { legacyGeoAdmin1Id: id }]
      : level === 2
        ? [{ id }, { legacyGeoAdmin2Id: id }]
        : [{ id }, { legacyGeoAdmin3Id: id }];
...
export async function actionListGeoAdmin1(input: {
  ...
  const rows = await prisma.geoDivision.findMany({
    ...
    orderBy: [{ name: "asc" }],
    take: limit,
    select: { id: true, code: true, name: true, isActive: true, legacyGeoAdmin1Id: true }
  });
...
export async function actionListGeoAdmin2(input: {
  ...
  const division1 = await resolveGeoDivisionFromAdminLikeId({
    id: admin1Id,
    level: 1,
    onlyActive: input.onlyActive
  });
  if (!division1) return { items: [] };
  ...
  const rows = await prisma.geoDivision.findMany({
    ...
    orderBy: [{ name: "asc" }],
    take: limit,
    select: { id: true, code: true, name: true, isActive: true, legacyGeoAdmin2Id: true }
  });
...
export async function actionListGeoAdmin3(input: {
  ...
  const q = input.q?.trim() ?? "";
  const limit = Math.min(Math.max(input.limit ?? 120, 20), 500);
  const division2 = await resolveGeoDivisionFromAdminLikeId({
    id: admin2Id,
    level: 2,
    onlyActive: input.onlyActive
  });
  if (!division2) return { items: [] };
  ...
  const rows = await prisma.geoDivision.findMany({
    ...
    orderBy: [{ name: "asc" }],
    take: limit,
    select: { id: true, code: true, name: true, isActive: true, legacyGeoAdmin3Id: true }
  });
...
export async function actionAddClientLocation(input: {
  ...
  if (input.type === ClientLocationType.FISCAL && !postalCode) {
    throw new Error("Código postal requerido para ubicación fiscal.");
  }
  const geo = await resolveGeoSelection({
    geoCountryId: input.geoCountryId,
    geoAdmin1Id: input.geoAdmin1Id,
  ...
  if (input.type === ClientLocationType.FISCAL && !geo.geoCountryId) {
    throw new Error("País requerido para ubicación fiscal.");
  }
...
export async function actionAddClientContact(input: {
  ...
  const role = normalizeOptional(input.role);
  const parentClient = await prisma.clientProfile.findUnique({
    where: { id: clientId },
    select: { country: true }
  });
  const phoneE164 = await normalizePhoneE164ForWrite({
    rawPhone: input.phone,
    explicitIso2: input.phoneCountryIso2,
    countryText: parentClient?.country ?? null,
...
export async function actionUpdateClientBasics(input: {
  ...
  const phoneE164 = await normalizePhoneE164ForWrite({
    rawPhone: input.phone,
    explicitIso2: input.phoneCountryIso2,
    countryText: normalizedCountry ?? existing.country,
    fieldLabel: "Teléfono"
  });
