...
function channelLabel(channel: PortalAppointmentRequestRow["channel"]) {
...
function channelBadgeClasses(channel: PortalAppointmentRequestRow["channel"]) {
    ...
    <section className="space-y-4 rounded-2xl border border-[#dce7f5] bg-white p-5 shadow-sm">
      ...
      {rows.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-xl border border-[#dce7f5]">
          <table className="min-w-[1280px] w-full divide-y divide-[#e2ebf8]">
            ...
            <tbody className="divide-y divide-[#eef3fb] bg-white">
              {rows.map((row) => (
                <tr key={row.id} className="align-top">
                  ...
                  <td className="px-3 py-3">
                    <span className={`inline-flex rounded-full border px-2 py-1 text-xs font-semibold ${channelBadgeClasses(row.channel)}`}>
                      {channelLabel(row.channel)}
                    </span>
                  </td>
                  <td className="px-3 py-3 text-sm text-slate-700">{row.companyName || "—"}</td>
        ...
        {confirmTarget ? (
          <form id="confirm-portal-request-form" onSubmit={handleConfirm} className="space-y-4">
            <div className="rounded-xl border border-[#dce7f5] bg-[#f8fbff] px-3 py-2 text-sm text-slate-700">
              ...
              <p className="mt-1">
                <span className="font-semibold text-[#102a43]">Canal:</span> {channelLabel(confirmTarget.channel)}
              </p>
              <p className="mt-1">
                <span className="font-semibold text-[#102a43]">Empresa:</span> {confirmTarget.companyName || "—"}
              </p>
              <p className="mt-1">
