...
export default function EmployeesPage() {
  ...
  const [archiveForm, setArchiveForm] = useState<{ effectiveDate: string; notes: string }>({
    effectiveDate: todayInputValue(),
    notes: ""
  });
  ...
  const canChangeStatus = hasPermission("HR:EMPLOYEES:STATUS");
  ...
  const quickCreateNextAction = useRef<"CLOSE" | "WIZARD">("CLOSE");
  ...
  useEffect(() => {
    ...
    if (!canUploadRestricted && uploadDocForm.visibility === "RESTRINGIDO") {
      setUploadDocForm((f) => ({ ...f, visibility: "PERSONAL" }));
    }
  }, [activeModal, selectedEmployeeId, canUploadRestricted, uploadDocForm.visibility]);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (payload) => {
      ...
      const employeeId = payload?.data?.id || payload?.id;
      setShowCreate(false);
      setCreateForm(emptyCreateForm(branchesQuery.data?.[0]?.id));
      if (quickCreateNextAction.current === "WIZARD") {
        if (employeeId) {
          router.push(`/hr/employees/new?id=${employeeId}`);
        } else {
          showToast("No se pudo abrir el expediente (id faltante)", "error");
        }
      }
      if (quickCreateNextAction.current === "CLOSE") {
        const actions = [
          employeeId ? { label: "Completar ahora", href: `/hr/employees/new?id=${employeeId}` } : undefined,
          { label: "Ver pendientes", href: "/hr/employees/pending" }
        ].filter(Boolean) as { label: string; href: string }[];
        showToast("Colaborador creado como borrador. Completa el expediente si faltan datos.", "info", actions);
      }
      quickCreateNextAction.current = "CLOSE";
    },
    onError: (err: any) => {
      quickCreateNextAction.current = "CLOSE";
  ...
  const handleQuickCreate = (action: "CLOSE" | "WIZARD") => {
    quickCreateNextAction.current = action;
  ...
  const archiveMutation = useMutation({
    mutationFn: async (payload: { effectiveDate: string; notes?: string | null }) => {
      if (!selectedEmployeeId) throw new Error("Sin empleado");
      const res = await fetch(`/api/hr/employees/${selectedEmployeeId}/archive`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload)
      });
    ...
    onError: (err: any) => {
      const message = err?.message || "Error al archivar";
      setErrorMsg(message);
      showToast(message, "error");
    }
  });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">Gestiona activos, pendientes y archivados desde las pestañas.</p>
          <div className="mt-2">
            <EmployeesTabs active="active" />
          </div>
        </div>
                        ...
                        <div className="relative">
                          ...
                          {actionMenu === emp.id && (
                            <div className="absolute right-0 z-10 mt-1 w-44 rounded-xl border border-slate-200 bg-white shadow-lg text-sm">
                              ...
                              {canChangeStatus && emp.status !== "SUSPENDED" && emp.status !== "TERMINATED" && (
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setActionMenu(null);
                              ...
                              {canChangeStatus && emp.status !== "ARCHIVED" && emp.status !== "TERMINATED" && (
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setActionMenu(null);
                                    setArchiveForm({ effectiveDate: todayInputValue(), notes: "" });
                                    openDetail(emp.id, "ARCHIVE");
                                  }}
                                  className="block w-full px-3 py-2 text-left hover:bg-slate-50"
                                >
                                  Archivar
                              ...
                              {canChangeStatus && emp.status !== "TERMINATED" && (
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setActionMenu(null);
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <button
              onClick={() => handleQuickCreate("CLOSE")}
            ...
            <button
              onClick={() => handleQuickCreate("WIZARD")}
              ...
              disabled={createMutation.isPending}
            >
              {createMutation.isPending ? "Guardando..." : "Guardar y completar expediente"}
            </button>
          </div>
        }
      >
        <div className="space-y-4 text-sm">
          <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
            <span>¿Falta información? Completa el expediente (jornada, compensación y documentos).</span>
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <button
              onClick={() => {
                if (!archiveForm.effectiveDate) {
                  showToast("Selecciona la fecha de archivo", "error");
                  return;
                }
                archiveMutation.mutate({
                  effectiveDate: archiveForm.effectiveDate,
                  notes: archiveForm.notes?.trim() || null
                });
              }}
              className="rounded-xl bg-slate-800 px-4 py-2 text-sm font-semibold text-white shadow-soft hover:-translate-y-px transition disabled:opacity-60"
              disabled={archiveMutation.isPending}
            >
              {archiveMutation.isPending ? "Archivando..." : "Confirmar archivo"}
            </button>
          </div>
        }
      >
        <div className="space-y-3 text-sm">
          <p className="text-slate-600">
            El colaborador pasará a la pestaña de Archivados. No se eliminan datos y puedes reactivarlo si es necesario.
          </p>
          <div className="space-y-1">
            <label className="text-slate-600">
              Fecha de archivo <span className="text-rose-500">*</span>
            </label>
            <input
              type="date"
              value={archiveForm.effectiveDate}
              onChange={(e) => setArchiveForm((f) => ({ ...f, effectiveDate: e.target.value }))}
              className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 shadow-inner focus:outline-none focus:ring-2 focus:ring-brand-primary/30"
            />
          </div>
          <div className="space-y-1">
            ...
            <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-gradient-to-r from-sky-50 to-white px-4 py-3 print-card">
              <div className="flex items-center gap-3">
                {identityLogoUrl ? (
                  ...
                  <div className="flex h-12 w-12 items-center justify-center rounded-full bg-brand-primary/10 text-brand-primary text-sm font-bold border border-brand-primary/30">
                    {selectedEmployee ? initialsFromName(selectedEmployee.fullName) : identityInitials}
                  </div>
                )}
                <div>
                  <p className="text-lg font-semibold text-slate-900 leading-tight">{identityName}</p>
