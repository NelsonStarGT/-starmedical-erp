...
import { CategoriaServicio, Subcategoria } from "@/lib/types/inventario";
import { cn } from "@/lib/utils";
import { categoriasServicioMock } from "@/lib/mock/inventario-catalogos";

type Props = { role?: string };

export function ServiceCategoriesManager({ role = "Administrador" }: Props) {
  ...
  const [catForm, setCatForm] = useState<Partial<CategoriaServicio>>({
    area: categoriasServicioMock[0]?.area || "CONSULTA",
    estado: "Activo",
    order: 0
  });
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [cRes, sRes] = await Promise.all([
        fetch("/api/inventario/categorias/servicios"),
        fetch("/api/inventario/subcategorias/servicios")
      ]);
  ...
  const saveCategory = async () => {
    if (!catForm.nombre || !catForm.slug || !catForm.area) {
      alert("Nombre, slug y área son requeridos");
      return;
    }
    const payload = {
      name: catForm.nombre,
    ...
    const res = await fetch(catForm.id ? `/api/inventario/categorias/servicios/${catForm.id}` : "/api/inventario/categorias/servicios", {
    ...
    await load();
    setCatForm({ area: categoriasServicioMock[0]?.area || "CONSULTA", estado: "Activo", order: 0 });
  };

  const deleteCategory = async (id: string) => {
    if (!confirm("¿Eliminar categoría? (solo si no tiene dependencias)")) return;
    const res = await fetch(`/api/inventario/categorias/servicios/${id}`, { method: "DELETE", headers: { "x-role": role } });
    const data = await res.json();
    if (!res.ok) {
  ...
  const saveSubcategory = async () => {
    ...
    const res = await fetch(
      subForm.id ? `/api/inventario/subcategorias/servicios/${subForm.id}` : "/api/inventario/subcategorias/servicios",
      {
  ...
  const deleteSubcategory = async (id: string) => {
    ...
    const res = await fetch(`/api/inventario/subcategorias/servicios/${id}`, { method: "DELETE", headers: { "x-role": role } });
    const data = await res.json();
    if (!res.ok) {
      alert(data.error || "No se pudo eliminar");
      return;
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div className="lg:col-span-2 overflow-auto rounded-2xl border border-slate-200 bg-white shadow-soft">
        <div className="flex items-center justify-between px-3 py-2 border-b border-slate-100">
          <p className="text-sm font-semibold text-slate-800">Categorías de servicios</p>
        ...
        <table className="min-w-full divide-y divide-slate-200">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-3 py-2 text-left text-xs font-semibold text-slate-500 uppercase">Área</th>
          ...
          <tbody className="divide-y divide-slate-200 bg-white">
            {categories.map((c) => (
              ...
              >
                ...
                <td className="px-3 py-2 text-sm text-slate-700">{c.area}</td>
      ...
      <div className="space-y-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-soft space-y-3">
          ...
          <select
            value={catForm.area || ""}
            onChange={(e) => setCatForm({ ...catForm, area: e.target.value })}
            className="w-full rounded-xl border border-[#E5E5E7] px-3 py-2 text-sm"
          >
            {categoriasServicioMock.map((c) => (
          ...
          <div className="flex justify-end gap-2">
            {catForm.id && (
              <button className="text-xs text-slate-500 underline" onClick={() => setCatForm({ area: categoriasServicioMock[0]?.area || "CONSULTA", estado: "Activo", order: 0 })}>
                Limpiar
              </button>
            )}
            <button
