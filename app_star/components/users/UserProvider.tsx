...
type UserContextValue = {
  ...
  addRoleOperativo: (role: Role) => void;
...
const rolesSeed: Role[] = [
  ...
  { id: 11, nombre: "SUPER_ADMIN", descripcion: "Acceso total al ERP", tipo: "Administrativo", estado: "Activo" }
];
...
const usuariosSeed: Usuario[] = [
  ...
  },
  {
    ...
    segundoApellido: "Allen",
    correo: "nelsonlopezallen@gmail.com",
    fotoUrl: "",
    rolOperativoId: 11,
...
export function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  const [rolesOperativos, setRolesOperativos] = useState<Role[]>(rolesSeed);
  const [rolesSistema] = useState<string[]>(["SUPER_ADMIN", "ADMIN", "HR_ADMIN", "HR_USER", "STAFF", "VIEWER"]);
  ...
  const addRoleOperativo = (role: Role) => setRolesOperativos((prev) => [...prev, role]);
  ...
  const value = useMemo(
    ...
    [usuarios, rolesOperativos, rolesSistema, sucursales, tiposContrato, tiposJornada, documentosDef]
  );
