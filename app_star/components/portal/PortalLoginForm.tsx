...
export function PortalLoginForm() {
  ...
  const canSubmit = useMemo(() => {
    return dpi.trim().length >= 6 && phone.trim().length >= 8;
  }, [dpi, phone]);
  ...
  const submitRequest = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setState("success");
      setMessage(String(payload?.message || "Si los datos coinciden, te enviaremos un acceso temporal."));
      setCooldown(Number(payload?.cooldownSeconds || 60));
      setNextStepUrl(String(payload?.nextStepUrl || `/portal/auth/verify?phone=${encodeURIComponent(phone.trim())}`));
  ...
  return (
    <form onSubmit={submitRequest} className="space-y-4">
      ...
      <p className="text-xs text-slate-500">
        Si los datos coinciden, te enviaremos un acceso temporal.
      </p>
      ...
      {state === "success" && nextStepUrl ? (
        <div className="rounded-xl border border-[#d7e6f8] bg-[#f8fbff] px-4 py-3">
          <p className="text-sm font-semibold text-slate-800">¿Tienes tu código OTP?</p>
          <p className="mt-1 text-xs text-slate-600">Puedes validarlo ahora con tu teléfono.</p>
          <Link
            href={nextStepUrl}
            className="mt-3 inline-flex items-center justify-center rounded-xl bg-[#2e75ba] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#245f96]"
          >
            Ingresar código
          </Link>
        </div>
