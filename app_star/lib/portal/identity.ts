...
export async function resolvePortalPersonByIdentity(input: {
  ...
  if (normalizedPhone) {
    ...
    if (exactPhoneMatches.length > 1) {
      if (normalizedEmail) {
        const exactEmailMatches = byExactEmail(exactPhoneMatches, normalizedEmail);
        if (exactEmailMatches.length === 1) return exactEmailMatches[0];

        const partialEmailMatches = byPartialEmail(exactPhoneMatches, normalizedEmail);
  ...
  // Fallback secundario: si el teléfono no coincide o no resuelve, intenta por email.
  if (normalizedEmail) {
