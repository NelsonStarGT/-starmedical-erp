// Catálogo base de permisos y roles. Mantener sincronizado con módulos CRM/HR.

export const CRM_PERMISSIONS = [
  "CRM:LEADS:READ",
  "CRM:LEADS:WRITE",
  "CRM:DEALS:READ",
  "CRM:DEALS:WRITE",
  "CRM:QUOTES:READ",
  "CRM:QUOTES:WRITE",
  "CRM:QUOTES:PUBLISH",
  "CRM:QUOTES:APPROVE",
  "CRM:PROPOSALS:READ",
  "CRM:PROPOSALS:WRITE",
  "CRM:PROPOSALS:PUBLISH",
  "CRM:FILES:READ",
  "CRM:FILES:WRITE",
  "CRM:SETTINGS:ADMIN",
  "CRM:AUDIT:READ"
] as const;

export const MEMBERSHIP_PERMISSIONS = [
  "MEMBERSHIPS:READ",
  "MEMBERSHIPS:WRITE",
  "MEMBERSHIPS:ADMIN",
  "MEMBERSHIPS:PRICING:VIEW",
  "MEMBERSHIPS:PAYMENTS:ADMIN",
  "MEMBERSHIPS:DASHBOARD",
  "MEMBERSHIPS:CONTRACTS:READ",
  "MEMBERSHIPS:CONTRACTS:WRITE",
  "MEMBERSHIPS:PAYMENTS:WRITE",
  "MEMBERSHIPS:PLANS:READ",
  "MEMBERSHIPS:PLANS:WRITE",
  "MEMBERSHIPS:CONFIG:READ",
  "MEMBERSHIPS:CONFIG:WRITE",
  "MEMBERSHIPS:USAGE:READ"
] as const;

export const DIAGNOSTIC_PERMISSIONS = [
  "DIAG:READ",
  "DIAG:WRITE",
  "DIAG:VALIDATE",
  "DIAG:RELEASE",
  "DIAG:RADIOLOGY"
] as const;

export const LABTEST_PERMISSIONS = [
  "LABTEST:READ",
  "LABTEST:WRITE",
  "LABTEST:VALIDATE",
  "LABTEST:RELEASE",
  "LABTEST:SEND",
  "LABTEST:ADMIN"
] as const;

export const HR_PERMISSIONS = [
  "HR:EMPLOYEES:READ",
  "HR:EMPLOYEES:WRITE",
  "HR:EMPLOYEES:STATUS",
  "HR:DOCS:READ",
  "HR:DOCS:UPLOAD",
  "HR:DOCS:RESTRICTED",
  "HR:ATTENDANCE:READ",
  "HR:ATTENDANCE:WRITE",
  "HR:ATTENDANCE:CLOSE",
  "HR:DASHBOARD:READ",
  "HR:PAYROLL:READ",
  "HR:PAYROLL:WRITE",
  "HR:PAYROLL:APPROVE",
  "HR:PAYROLL:PUBLISH",
  "HR:SETTINGS:READ",
  "HR:SETTINGS:WRITE"
] as const;

export const RECEPTION_PERMISSIONS = [
  "RECEPTION_VIEW",
  "RECEPTION_QUEUE_VIEW",
  "RECEPTION_QUEUE_WRITE",
  "RECEPTION_APPOINTMENTS_VIEW",
  "RECEPTION_APPOINTMENTS_WRITE",
  "RECEPTION_ADMISSIONS_VIEW",
  "RECEPTION_ADMISSIONS_WRITE",
  "RECEPTION_CASHIER_VIEW",
  "RECEPTION_CASHIER_WRITE",
  "RECEPTION_REGISTRATIONS_VIEW",
  "RECEPTION_REGISTRATIONS_WRITE"
] as const;

export const CLIENTS_BULK_PERMISSIONS = [
  "CLIENTS_EXPORT_TEMPLATE",
  "CLIENTS_EXPORT_DATA",
  "CLIENTS_IMPORT_ANALYZE",
  "CLIENTS_IMPORT_PROCESS",
  "CLIENTS_IMPORT_PROCESS_UPDATE"
] as const;

export const CLIENTS_REPORTS_PERMISSIONS = [
  "CLIENTS_REPORTS_VIEW",
  "CLIENTS_REPORTS_EXPORT",
  "CLIENTS_REPORTS_EXPORT_FULL",
  "CLIENTS_REPORTS_EXPORT_MASKED"
] as const;

export const FINANCE_PERMISSIONS = [
  "FINANCE:READ",
  "FINANCE:WRITE",
  "FINANCE:POST"
] as const;

export const FILE_PERMISSIONS = ["FILES:UPLOAD"] as const;

export const COMMUNICATIONS_PERMISSIONS = [
  "INTEGRATIONS:WHATSAPP:READ",
  "INTEGRATIONS:WHATSAPP:SEND"
] as const;

export const SYSTEM_PERMISSIONS = [
  "USERS:ADMIN",
  "SYSTEM:ADMIN",
  "CONFIG_BRANCH_READ",
  "CONFIG_BRANCH_WRITE",
  "CONFIG_THEME_READ",
  "CONFIG_THEME_WRITE",
  "CONFIG_NAVIGATION_READ",
  "CONFIG_NAVIGATION_WRITE",
  "CONFIG_SAT_READ",
  "CONFIG_SAT_WRITE",
  "CONFIG_BILLING_READ",
  "CONFIG_BILLING_WRITE",
  "CONFIG_EMAIL_READ",
  "CONFIG_EMAIL_WRITE",
  "CONFIG_EMAIL_SANDBOX_READ",
  "CONFIG_SERVICES_READ",
  "CONFIG_SERVICES_WRITE",
  "CONFIG_SECURITY_READ",
  "CONFIG_SECURITY_WRITE",
  "CONFIG_AUDIT_READ",
  "CONFIG_API_READ",
  "CONFIG_API_WRITE",
  "CONFIG_BACKUP_READ",
  "CONFIG_BACKUP_WRITE"
] as const;

export const ALL_PERMISSION_KEYS: string[] = [
  ...CRM_PERMISSIONS,
  ...MEMBERSHIP_PERMISSIONS,
  ...DIAGNOSTIC_PERMISSIONS,
  ...LABTEST_PERMISSIONS,
  ...HR_PERMISSIONS,
  ...RECEPTION_PERMISSIONS,
  ...CLIENTS_BULK_PERMISSIONS,
  ...CLIENTS_REPORTS_PERMISSIONS,
  ...FINANCE_PERMISSIONS,
  ...FILE_PERMISSIONS,
  ...COMMUNICATIONS_PERMISSIONS,
  ...SYSTEM_PERMISSIONS
];

export const ROLE_PERMISSION_MAP: Record<string, string[]> = {
  SUPER_ADMIN: ALL_PERMISSION_KEYS,
  ADMIN: ALL_PERMISSION_KEYS,
  HR_ADMIN: [
    ...HR_PERMISSIONS,
    ...DIAGNOSTIC_PERMISSIONS,
    ...MEMBERSHIP_PERMISSIONS,
    ...FILE_PERMISSIONS,
    "USERS:ADMIN"
  ],
  HR_USER: [
    "HR:EMPLOYEES:READ",
    "HR:EMPLOYEES:WRITE",
    "HR:DASHBOARD:READ",
    "HR:DOCS:READ",
    "HR:DOCS:UPLOAD",
    "HR:ATTENDANCE:READ",
    "HR:ATTENDANCE:WRITE",
    "HR:PAYROLL:READ",
    ...FILE_PERMISSIONS,
    "DIAG:READ",
    "DIAG:WRITE"
  ],
  STAFF: [
    "HR:EMPLOYEES:READ",
    "HR:DOCS:READ",
    ...FILE_PERMISSIONS,
    "CLIENTS_REPORTS_VIEW",
    "CLIENTS_REPORTS_EXPORT",
    "CLIENTS_REPORTS_EXPORT_MASKED"
  ],
  VIEWER: ["HR:EMPLOYEES:READ"],
  SUPERVISOR: [
    "CRM:LEADS:READ",
    "CRM:LEADS:WRITE",
    "CRM:DEALS:READ",
    "CRM:DEALS:WRITE",
    "CRM:QUOTES:READ",
    "CRM:QUOTES:WRITE",
    "CRM:QUOTES:PUBLISH",
    "CRM:QUOTES:APPROVE",
    "CRM:PROPOSALS:READ",
    "CRM:PROPOSALS:WRITE",
    "CRM:PROPOSALS:PUBLISH",
    "CRM:FILES:READ",
    "CRM:FILES:WRITE",
    "CRM:SETTINGS:ADMIN",
    "CRM:AUDIT:READ",
    ...FILE_PERMISSIONS,
    ...COMMUNICATIONS_PERMISSIONS
  ],
  SALES: [
    "CRM:LEADS:READ",
    "CRM:LEADS:WRITE",
    "CRM:DEALS:READ",
    "CRM:DEALS:WRITE",
    "CRM:QUOTES:READ",
    "CRM:QUOTES:WRITE",
    "CRM:QUOTES:PUBLISH",
    "CRM:PROPOSALS:READ",
    "CRM:PROPOSALS:WRITE",
    "CRM:PROPOSALS:PUBLISH",
    "CRM:FILES:READ",
    "CRM:FILES:WRITE",
    ...FILE_PERMISSIONS,
    ...COMMUNICATIONS_PERMISSIONS
  ],
  RECEPTION: [
    "CRM:LEADS:READ",
    "CRM:DEALS:READ",
    "CRM:QUOTES:READ",
    "RECEPTION_VIEW",
    "RECEPTION_QUEUE_VIEW",
    "RECEPTION_QUEUE_WRITE",
    "RECEPTION_APPOINTMENTS_VIEW",
    "RECEPTION_APPOINTMENTS_WRITE",
    "RECEPTION_ADMISSIONS_VIEW",
    "RECEPTION_ADMISSIONS_WRITE",
    "RECEPTION_CASHIER_VIEW",
    "RECEPTION_REGISTRATIONS_VIEW",
    ...FILE_PERMISSIONS,
    ...COMMUNICATIONS_PERMISSIONS
  ],
  RECEPTIONIST: [
    "RECEPTION_VIEW",
    "RECEPTION_QUEUE_VIEW",
    "RECEPTION_QUEUE_WRITE",
    "RECEPTION_APPOINTMENTS_VIEW",
    "RECEPTION_APPOINTMENTS_WRITE",
    "RECEPTION_ADMISSIONS_VIEW",
    "RECEPTION_ADMISSIONS_WRITE",
    "RECEPTION_CASHIER_VIEW",
    "RECEPTION_REGISTRATIONS_VIEW",
    ...FILE_PERMISSIONS,
    ...COMMUNICATIONS_PERMISSIONS
  ],
  CASHIER: [
    "RECEPTION_VIEW",
    "RECEPTION_CASHIER_VIEW",
    "RECEPTION_CASHIER_WRITE",
    "RECEPTION_ADMISSIONS_VIEW",
    "RECEPTION_REGISTRATIONS_VIEW",
    ...FILE_PERMISSIONS
  ],
  OPS: [
    "RECEPTION_VIEW",
    "RECEPTION_QUEUE_VIEW",
    "RECEPTION_QUEUE_WRITE",
    "RECEPTION_APPOINTMENTS_VIEW",
    "RECEPTION_APPOINTMENTS_WRITE",
    "RECEPTION_ADMISSIONS_VIEW",
    "RECEPTION_ADMISSIONS_WRITE",
    "RECEPTION_CASHIER_VIEW",
    "RECEPTION_REGISTRATIONS_VIEW",
    "CLIENTS_EXPORT_TEMPLATE",
    "CLIENTS_EXPORT_DATA",
    "CLIENTS_IMPORT_ANALYZE",
    "CLIENTS_IMPORT_PROCESS",
    "CLIENTS_REPORTS_VIEW",
    "CLIENTS_REPORTS_EXPORT",
    "CLIENTS_REPORTS_EXPORT_FULL",
    "CLIENTS_REPORTS_EXPORT_MASKED",
    ...FILE_PERMISSIONS,
    ...COMMUNICATIONS_PERMISSIONS
  ],
  TENANT_ADMIN: [
    "RECEPTION_VIEW",
    "RECEPTION_QUEUE_VIEW",
    "RECEPTION_QUEUE_WRITE",
    "RECEPTION_APPOINTMENTS_VIEW",
    "RECEPTION_APPOINTMENTS_WRITE",
    "RECEPTION_ADMISSIONS_VIEW",
    "RECEPTION_ADMISSIONS_WRITE",
    "RECEPTION_CASHIER_VIEW",
    "RECEPTION_CASHIER_WRITE",
    "RECEPTION_REGISTRATIONS_VIEW",
    "RECEPTION_REGISTRATIONS_WRITE",
    "CLIENTS_REPORTS_VIEW",
    "CLIENTS_REPORTS_EXPORT",
    "CLIENTS_REPORTS_EXPORT_MASKED",
    ...FILE_PERMISSIONS,
    ...COMMUNICATIONS_PERMISSIONS
  ],
  FINANCE: [
    ...FINANCE_PERMISSIONS,
    ...FILE_PERMISSIONS,
    "CRM:QUOTES:READ",
    "CRM:QUOTES:APPROVE"
  ],
  LAB_TECH: ["LABTEST:READ", "LABTEST:WRITE", "LABTEST:VALIDATE", ...FILE_PERMISSIONS],
  LAB_SUPERVISOR: ["LABTEST:READ", "LABTEST:WRITE", "LABTEST:VALIDATE", "LABTEST:RELEASE", "LABTEST:SEND", ...FILE_PERMISSIONS],
  LAB_ADMIN: [...LABTEST_PERMISSIONS, ...FILE_PERMISSIONS]
};

export const BASE_ROLES = {
  ADMIN: {
    permissions: ROLE_PERMISSION_MAP.ADMIN
  },
  STAFF: {
    permissions: ROLE_PERMISSION_MAP.STAFF
  }
} as const;

export const ROLE_CATALOG = [
  "SUPER_ADMIN",
  "ADMIN",
  "OPS",
  "TENANT_ADMIN",
  "HR_ADMIN",
  "HR_USER",
  "STAFF",
  "VIEWER",
  "RECEPTIONIST",
  "CASHIER",
  "LAB_TECH",
  "LAB_SUPERVISOR",
  "LAB_ADMIN"
] as const;

export const ONBOARDING_ASSIGNABLE_ROLES = ["STAFF"] as const;
