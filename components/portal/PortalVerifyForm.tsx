...
export function PortalVerifyForm() {
  ...
  const verifyAccess = async (payload: { token?: string; destination?: string; code?: string }) => {
    setState("loading");
    setMessage(null);
  ...
  const submitOtp = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!destination.trim() || !/^\d{6}$/.test(code.trim())) {
      setState("error");
      setMessage("Ingresa destino y un código de 6 dígitos.");
      return;
    }
        ...
        <form onSubmit={submitOtp} className="space-y-4">
          <label className="block text-sm font-medium text-slate-700">
            ...
            <input
              ...
              placeholder="paciente@correo.com o 50255558888"
              autoComplete="off"
            />
          </label>
